package com.alura.screensound.principal;

import com.alura.screensound.model.Artista;
import com.alura.screensound.model.Cancion;
import com.alura.screensound.model.TipoArtista;
...
//    }
    ...
    private void buscarDatosDelArtista() {
        System.out.println("¿Desea buscar datos sobre qué artista?");
        var nombre = teclado.nextLine();

        Optional<Artista> artista = repositorio.findByNombreIgnoreCase(nombre);

        if (artista.isPresent()) {
            Artista encontrado = artista.get();
            System.out.printf("Artista: %s\nTipo: %s\nCanciones:\n", encontrado.getNombre(), encontrado.getTipo());
            encontrado.getCanciones().forEach(c -> System.out.printf(" - %s\n", c.getTitulo()));
        } else {
